 ...
 *
 */
public class G_NewLowBarHigh extends CommandGroup {
    
    public  G_NewLowBarHigh() {
    	NavX.getInstance().resetAngle();
    	DriveTrain.getInstance().resetSensors();
    	addParallel(new A_NewResetTurn(0.5));
    	addSequential(new A_SetIntakePosition(Intake.intakeState.DEAD, 1));
    	addSequential(new A_NewPositionMove(-212,5,-1));
//    	addParallel(new A_SetIntakePosition(Intake.intakeState.UP, 1));
//    	addSequential(new A_ResetTurn(0.5));
//    	addSequential(new A_NewPositionLock(63, 2));
//    	addSequential(new A_NewPositionMove(-69,2,1));
