 ...
 */
public class PIDController610 implements PIDInterface, LiveWindowSendable, Controller {
  ...
  private ArrayList<Double> m_buf;
  ...
  private class PIDTask extends TimerTask {
    ...
    private PIDController610 m_controller;

    public PIDTask(PIDController610 controller) {
      if (controller == null) {
   ...
   *        default is 50ms.
   */
  public PIDController610(double Kp, double Ki, double Kd, double Kf, PIDSource source,
   ...
   *        default is 50ms.
   */
  public PIDController610(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output,
      double period) {
    ...
    if (enabled) {
      ...
      synchronized (this) {
        ...
        if (m_buf.size() > m_bufLength) {
          m_bufTotal -= m_buf.remove(m_buf.size()-1);
        }
      }
